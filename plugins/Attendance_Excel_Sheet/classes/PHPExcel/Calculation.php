...
use plugins\Attendance_Excel_Sheet\classes\PHPExcel\CalcEngine\PHPExcel_CalcEngine_CyclicReferenceStack;
use plugins\Attendance_Excel_Sheet\classes\PHPExcel\CalcEngine\PHPExcel_CalcEngine_Logger;
use plugins\Attendance_Excel_Sheet\classes\PHPExcel\Calculation\PHPExcel_Calculation_Exception;
use plugins\Attendance_Excel_Sheet\classes\PHPExcel\Calculation\PHPExcel_Calculation_Function;
use plugins\Attendance_Excel_Sheet\classes\PHPExcel\Calculation\PHPExcel_Calculation_Functions;
use plugins\Attendance_Excel_Sheet\classes\PHPExcel\Calculation\Token\PHPExcel_Calculation_Token_Stack;
...
use plugins\Attendance_Excel_Sheet\classes\PHPExcel\Shared\JAMA\PHPExcel_Shared_JAMA_Matrix;
...
//echo 'Element is a Closing bracket', PHP_EOL;
                ...
                $d = $stack->last(2);
                // FJ fix PHP8.1 deprecated error trying to access array offset on value of type null
                if (!empty($d['value'])
                    && preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i', $d['value'], $matches)) {    //    Did this parenthesis just close a function?
...
//                    echo 'Element '.$val.' is a Cell reference<br />';
                    ...
                    // FJ fix PHP8.1 error trying to access array offset on value of type null
                    if (!empty($testPrevOp['value'])
                        && $testPrevOp['value'] == ':') {
...
//            echo '<br />';
            ...
            if (is_float($token)) {
                // FJ fix PHP8.1 error implicit conversion from float to int loses precision
                $token = (int)$token;
            }
